...
EOD;
	...
	* */
	...
	$borrow = 0;
	$dept = 0;
	foreach($loan_status as $record){
		$remain = ($record->bamount) - ($record->pamount);
		if($remain <= 0){
			$status = 'Complete';
		}else{
			$status = 'Not Complete';
		...
		$html .= '<tr>
			...
			<td style = "border:1px solid black">'.number_format($record->bamount).'</td>
			<td style = "border:1px solid black">'.number_format($record->pamount).'</td>
			<td style = "border:1px solid black">'.number_format($remain).'</td>
	...
	$pdf->Ln(4);
	$pdf->writeHTML('Total amount of money borrowed in this report', true, false, true, false, 'R');
	$pdf->writeHTML(number_format($borrow).'/=', true, false, true, false, 'R');
	$pdf->Ln(4);
	...
	$pdf->Ln(4);
	$pdf->writeHTML('Total amount of dept remained in this report', true, false, true, false, 'R');
	$pdf->writeHTML(number_format($dept).'/=', true, false, true, false, 'R');
	$pdf->Ln(2);
