	...
	* */
	...
	foreach($grant_loan as $record){
		$html .= '<tr>
			...
			<td style = "border:1px solid black">'.ucwords($record->bregistered_by).'</td>
			<td style = "border:1px solid black">'.$record->date.'</td>
		</tr>';
		$value = $record->bamount;
		$sum += $value;
	}
	$html .= '</table>';
	echo $sum;
	...
	$pdf->Ln(4);
	$pdf->writeHTML('Total amount of granted loan in this report', true, false, true, false, 'R');
