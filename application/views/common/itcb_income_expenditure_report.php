    ...
    */
    ...
    $html = <<<EOD
    <h1>ITCB INCOME & EXPENDITURE  REPORT.</h1>
EOD;
    ...
    $pdf->Ln(8);
	$pdf->Ln(2);
	$pdf->writeHTML('Income.', true, false, true, false, 'L');
	/*$table = "
	<table>
			<tr>
	...
	* */
	...
	$count =1;
	$sum = 0;
	foreach($income_expenditure as $record){
		if($record->fcategory == 'income category'){
		$html .= '<tr>
			<td style = "border:1px solid black">'.$count.'</td>
			<td style = "border:1px solid black">'.$record->income_name.'</td>
			<td style = "border:1px solid black">'.number_format($record->iamount).'</td>
			<td style = "border:1px solid black">'.$record->payer.'</td>
			<td style = "border:1px solid black">'.$record->iregistered_by.'</td>
			<td style = "border:1px solid black">'.$record->irecord_date.'</td>
		</tr>';
		$value = $record->iamount;
		$sum += $value;
		$count++;
	}
	}
	...
	$pdf->Ln(4);
	$pdf->writeHTML('Total amount of income in this report', true, false, true, false, 'R');
	$line = $sum;
	$pdf->writeHTML(number_format($line).'/=', true, false, true, false, 'R');
	
	$pdf->Ln(4);
	$pdf->Ln(2);
	$pdf->writeHTML('Expenditure.', true, false, true, false, 'L');
	//craeting the table
	$html = '<table style = "border:1px solid black">';
	$html .= '<tr style = "background-color:#8B6914; color:#000000">
			<th>No</th>
	...
	$exp = 0;
	foreach($income_expenditure as $record){
		if($record->fcategory == 'exp category'){
		$html .= '<tr>
			<td style = "border:1px solid black">'.$count.'</td>
			<td style = "border:1px solid black">'.$record->exp_name.'</td>
			<td style = "border:1px solid black">'.number_format($record->amount).'</td>
			<td style = "border:1px solid black">'.$record->payee.'</td>
			<td style = "border:1px solid black">'.$record->eregistered_by.'</td>
	...
	echo $exp;
	$pdf->writeHTML($html, true, false, true, false, '');
	$pdf->writeHTML('Total amount of expenditure in this report', true, false, true, false, 'R');
	$line = $exp;
	$pdf->writeHTML(number_format($line).'/=', true, false, true, false, 'R');
	
	$pdf->Ln(4);
	$minus = $sum - $exp;
	$pdf->writeHTML('Total Balance', true, false, true, false, 'R');
	$line = $minus;
	$pdf->writeHTML(number_format($line).'/=', true, false, true, false, 'R');
